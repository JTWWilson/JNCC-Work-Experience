...
def get_indices_for_fields(sheet, fields):
    ...
    for field_name in break_down_fields(fields):
        if field_name is None:
            pass
        try:
            relations[field_name] = [sheet_field.strip('\n').strip() for sheet_field in list(sheet.values)[config['Analysis_Workbook']['Field_Row'] - 1]].index(field_name)
        except ValueError:
            pass
    return relations
...
if __name__ == '__main__':
    ...
    config = import_config('1515Sconfig.yaml')
    ...
    for index, field in enumerate(config['Fields']):
        new_sheet[ascii_uppercase[index] + '1'] = field
